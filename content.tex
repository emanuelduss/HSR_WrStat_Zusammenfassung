...
\begin{itemize}
  \item $F$ ist diskret: $E(X) = \sum_i x_i \cdot px(x_i)$ % TODO Verify
...
\begin{description}
  ...
  \item[Varianz] \[\operatorname{var}(X) = \frac{(a-b)^2}{12}\]
...
\begin{description}
  ...
  \item[Varianz] \[\operatorname{var}(X) = \frac{1}{a^2}\]
...
\begin{description}
  \item[Anwendung] Messfehler, Summe von vielen voneinander
    unabhängigenn Zufallsvariabeln, Approximation der Binomialverteilung
...
\subsection{Begriffe}
\begin{itemize}
  \item Schätzer: Ist $X$ eine Zufallsvariable mit Verteilung mit
    Parameter $\mu, \sigma^2, n, p, \lambda, \vartheta$, liefert der
    Schätzer die Formel für diese Parameter.
...
\begin{itemize}
  \item Konsistente Schätzer (fast immer erfüllt): Jeh mehr
    Stichproben, desto genauer am wahren Wert des Parameters:
    $\lim_{n \rightarrow \infty} \vartheta(X_1, \dots, X_n)$
  \item Erwartungstreue Schätzer: Bei vielen Durchführungen des
  Experiments bekommt man im Mittel den Wahren Wert.
\end{itemize}
