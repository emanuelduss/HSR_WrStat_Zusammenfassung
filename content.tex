...
\begin{itemize}
  ...
  \item Abstand ($\pm$) zum Erwartungswert (Standardabweichung): $\sqrt{\text{var}(x)}$
\end{itemize}
...
$\sigma^2$, dann ist
\[ Y = \frac{X-\mu}{\sigma} \]
eine neue Zufallsvariable $Y$ mit $E(Y) = 0$ und $\text{var}(Y) = 0$.
...
\[ \phi_Y(y) = \sigma \phi_X(y\sigma+\mu) \qquad \phi_X(x) =
  \frac{1}{\sigma} \phi_Y\left(\frac{x-\mu}\sigma\right) \]
...
\begin{description}
  ...
  \item[Median] \[\text{med}(X) = \frac{a+b}{2}\]
...
\begin{description}
  ...
  \item[Median] \[\text{med}(X) = \frac{1}{a}\log 2\]
...
\begin{description}
  ...
  \item[Dichtefunktion] \[ \phi(X) = \begin{cases} Cx^{-\alpha} & x \geq
...
\begin{itemize}
  ...
  \item Ist $X$ annähernd normalverteilt, gilt:
    $E(X) = \mu = np$ und $\text{var}(X) = \sigma^2 = np(1-p)$
\end{itemize}
Verteilfunktion:
